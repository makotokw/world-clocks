...
(function ($) {
  ...
  { // optoins
    ...
    $.fn.extend({
      ...
      editTimezone: function () {
        var $self = $(this), id = $self.parent().attr('id'),
          cbxid = id + '_dst', l = locales[id];
        var $tz = $('<div class="timezone"/>'),
          $select = $timezone.clone(),
          $check = $('<input id="' + cbxid + '" class="dst" type="checkbox"/>').
            attr('checked', l.dst == 1);
        ...
        $tz.append($check.change(onTimezoneChanged)).
          append(
            '<label for="' + cbxid + '">' + w.msg('DST_LABEL') + '</label>');
        $self.after($tz);
      },
    });
  ...
  function resize () {
    var $items = $('li', $list),
      count = $items.length,
      listWidth = ((count == 0) ? column : Math.min(column, count)) *
        (margin * 2 + radius * 2),
      itemWidth = radius * 2
    ;
  ...
  function addClock (locale) {
    var i = locales._length++, l = locale ||
      { label: w.msg('LOCAL_TIME'), offset: localeOffset, dst: 0 };
    var id = 'locale' + i,
  ...
  function changeDigitalFont (fontname) {
    ...
    $('head').
      append('<link href="http://fonts.googleapis.com/css?family=' + fontname +
